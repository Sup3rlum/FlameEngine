...
namespace FlameEncoder.Compilers
{
    ...
    public class MaterialCompiler : ITaskCompiler
    {
        ...
        public static byte[] FromChannels(byte[] srcRed, byte[] srcGreen, byte[] srcBlue, byte[] srcAlpha)
        {
        ...
        public void StartTask(CompilerTask task)
        {
            ...
            EncodeMaterial((Material)task.TextureMaps, memory);
        ...
        {
            // Diffuse Map
            var diffuse = material.Maps["Diffuse"].Data[0];
            var data = ImageToByteArray32(diffuse);
            WriteMap(memory, diffuse.Width, diffuse.Height, data);

            // Normal Map
            var normal = material.Maps["Normal"].Data[0];
            data = ImageToByteArray32(normal);
            WriteMap(memory, normal.Width, normal.Height, data);

            // Detail
            var height = material.Maps["Height"].Data[0];
            var rough = material.Maps["Roughness"].Data[0];
            var metal = material.Maps["Metallic"].Data[0];
            var ao = material.Maps["AO"].Data[0];
            ...
            var emissive = material.Maps["Emissive"].Data[0];
