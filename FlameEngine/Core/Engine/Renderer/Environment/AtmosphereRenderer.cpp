...
void AtmosphereRenderer::RenderSkySphere(FRICommandList& cmdList, const Camera& cameraRef, const DirectionalLight& dirLight)
{
	...
	cmdList.SetShaderPipeline(pipeline);
	{
		...
		cmdList.ResourceSubdata(SettingsBuffer.GPU, FRIUpdateDescriptor(&buffer, 0, sizeof(FAtmosphereConstantBuffer)));
	}
...
{
	cmdList.SetShaderConstantBuffer(9, SettingsBuffer.GPU);

	cmdList.BeginCompute();
	cmdList.EndCompute();
	cmdList.SetUnorderedAccessViewCS(0, 0);
	cmdList.SetConstantBufferViewCS(0, 0);
	cmdList.DispatchCompute(0, 0, 0);
