...
void VXGIRenderer::CreateResources(FRIContext* renderContext)
{
	...
	VXGIBuffer.GPU = cmdList.GetDynamic()->CreateConstantBuffer(FRICreationDescriptor(NULL, sizeof(FVXGIBuffer)));
	VoxelizeBuffer.GPU = cmdList.GetDynamic()->CreateConstantBuffer(FRICreationDescriptor(NULL, sizeof(FMatrix4) * 3));
	PropagationData.GPU = cmdList.GetDynamic()->CreateConstantBuffer(FRICreationDescriptor(NULL, sizeof(float)*4));
...
{
	/*cmdList.SetShaderConstantBuffer(UBO_SLOT_VOXELIZE, VoxelizeBuffer.GPU);
	cmdList.SetShaderConstantBuffer(UBO_SLOT_VXGI, VXGIBuffer.GPU);
	cmdList.SetShaderConstantBuffer(11, PropagationData.GPU);
