...
PSOutput main(PSInput input)
{
    ...
    float4 worldPos = mul(InverseView, input.ViewPosition);
    float height = length(worldPos.xyz);
    if (height < 1)
    {
        output.Albedo = float4(0.4, 0.5, 1.0, 1);
    }
    else
    {
        output.Albedo = float4(0.7, 0.7, 0.5, 1);
    }
    
    output.Albedo = pow(output.Albedo, 2.2);
    output.Detail = float4(Detail.gba, 1);
